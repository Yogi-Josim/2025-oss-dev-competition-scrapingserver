...
# [추가] 각 스레드마다 독립적인 Selenium 드라이버를 관리하기 위한 thread-local storage
thread_local = local()


def get_driver():
  """각 스레드에 대해 WebDriver 인스턴스를 생성하거나 기존 인스턴스를 반환합니다."""
  ...
  while not task_queue.empty():
    try:
      ...
      try:
        ...
        print(f"  [정보] requests 실패: {link}. Selenium으로 재시도합니다. ({e})")
